...
from nose.tools import assert_equals, assert_in, assert_not_equals, assert_raises
...
def test_unlimited_supply_remove():
    ...
    assert_equals(supply.count(CardType.BLUE), 2)
    assert_in(supply.remove(), [Card.WHEAT_FIELD, Card.MINE, Card.CAFE, Card.BAKERY])
    assert_equals(supply.remove(["|", CardType.RED, CardType.LANDMARK]), Card.CAFE)
...
def test_limited_supply_remove():
    ...
    assert_in(
        supply.remove(), [Card.WHEAT_FIELD, Card.MINE, Card.STADIUM, Card.STATION]
    )
    assert_equals(supply.remove(["&", CardType.BLUE, CardSymbol.GEAR]), Card.MINE)
    assert_raises(KeyError, supply.remove, ["&", CardType.RED, CardSymbol.GEAR])
...
def test_tiles_market_count():
    ...
    assert_equals(supply.count(CardType.BLUE) + market.count(CardType.BLUE), 30)
    assert_equals(supply.count(CardType.GREEN) + market.count(CardType.GREEN), 30)
    assert_equals(supply.count(CardType.RED) + market.count(CardType.RED), 12)
    assert_equals(supply.count(CardType.PURPLE) + market.count(CardType.PURPLE), 12)
    assert_equals(market.count(CardType.LANDMARK), 16)
    assert_equals(market.count(CardType.LANDMARK), supply.count(CardType.LANDMARK))
...
def test_tiles_market_remove():
    ...
    for i in range(7):
        assert_equals(
            supply.count(CardType.PURPLE) + market.count(CardType.PURPLE), 12 - i
        )
        assert_equals(len(market.list(CardType.PURPLE)), 2)
        assert_in(
            market.remove(CardType.PURPLE),
            [Card.STADIUM, Card.TV_STATION, Card.BUSINESS_CENTRE],
        )
        assert_equals(len(market.list(CardType.PURPLE)), 2)
        assert_equals(
...
def test_tableau_copy():
    ...
    assert_equals(tableau.count(Card.WHEAT_FIELD), tableau2.count(Card.WHEAT_FIELD))
    tableau2.money(12)
    ...
    assert_not_equals(tableau.count(Card.WHEAT_FIELD), tableau2.count(Card.WHEAT_FIELD))
