...
class Rules(object):
    class Phase(object):
        ...
        def __ask_player(card, game, answers):
            result = game.player(Game.CURRENT_PLAYER).decide(card, game, answers)
        ...
        def roll_dice(game):
            ...
            if game.list(Game.CURRENT_PLAYER, Card.RADIO_TOWER):
                # Re-Roll
                if Rules.Phase.__ask_player(Card.RADIO_TOWER, game, [True, False]):
                    game.roll(Game.DICE_GAME, dice_count)
            if (
                game.list(Game.CURRENT_PLAYER, Card.HARBOUR)
                and game.pip(Game.DICE_GAME) >= 10
            ):
                # Add 2?
        ...
        def __earn_income(game, owner, card_type):
            for card in game.list(owner, ["&", card_type, game.pip(Game.DICE_GAME)]):
        ...
        def earn_income(game):
            ...
            """ green (player) """
            Rules.Phase.__earn_income(
                game, game.player(Game.CURRENT_PLAYER), CardType.SECONDARY_INDUSTRY
            )
            """ blue (player + neighbors) """
            Rules.Phase.__earn_income(
                game, game.player(Game.CURRENT_PLAYER), CardType.PRIMARY_INDUSTRY
            )
            for player in game.neighbors(Game.CURRENT_PLAYER):
                Rules.Phase.__earn_income(game, player, CardType.PRIMARY_INDUSTRY)
            """ purple (player) """
            Rules.Phase.__earn_income(
                game, game.player(Game.CURRENT_PLAYER), CardType.MAJOR_ESTABLISHMENT
            )

        @staticmethod
        def construction(game):
            if (
                game.list(Game.CURRENT_PLAYER, Card.CITY_HALL)
                and game.money(Game.CURRENT_PLAYER) == 0
            ):
    ...
    def evaluate(game):
        while True:
            ...
            if (
                game.list(Game.CURRENT_PLAYER, Card.AMUSEMENT_PARK)
                and len(dice) > 1
                and dice[0] == dice[1]
            ):
                game.advance(False)
